...
(defmethod format-role "engineer" [params]
  {:color (shade (color :argvader-green) :darker 60)
   :background (->hex (color :argvader-green))})
...
(defmethod format-role "data science" [params]
  {:color (shade (color :argvader-green) :darker 60)
   :background (shade (color :argvader-green) :lighter 50)})
...
(def title
  {:background (->hex (color :argvader-purple))
   :color (->hex (color :white))
   :text-align "center"
   :height "100%"
   :padding "0.5em 0.3em"})
